...
public class TestInitLoadDirectory {
    ...
    public void testInitDirectoryWithAlways() throws Exception {
        ...
        hotDeployer.deploy("org.nuxeo.ecm.directory.tests:csv-always-autoincrement-directory-reject-duplicate-contrib.xml");
    ...
    public void testInitDirectoryWithNever() throws Exception {
        ...
        hotDeployer.deploy(
                "org.nuxeo.ecm.directory.tests:csv-never-directory-update-duplicate-contrib.xml");
        try (Session session = directoryService.open(CSV_LOAD_DIRECTORY)) {
    ...
    public void testInitDirectoryWithOnMissingColumns() throws Exception {
        ...
        hotDeployer.deploy(
                "org.nuxeo.ecm.directory.tests:csv-on-missing-columns-directory-never-load-contrib.xml");
        // We verify that nothing has changed
        assertDirectoryEntries();

        // Then we test with dataLoadingPolicy = ignore_duplicate
        hotDeployer.deploy(
                "org.nuxeo.ecm.directory.tests:csv-on-missing-columns-directory-skip-duplicate-contrib.xml");
